...
## 애니메이션 배지·연속 감상 추가 버전

이 버전은 배포된 기동 복구 패치와 문서 기준 `be6c625` 위에 GIF 배지와
상세보기 이전·다음 이동을 추가한다. 사용자가 `a9163851de8b`의 GIF 배지와
좌우 이동 동작을 확인했으며, 이번 버전은 이를 실제 애니메이션 GIF·WebP만
표시하도록 확장한다. 아래 기동 복구 이력도 포함한다.
현대화 작업·의존성 버전·원본 파일은 변경하지 않는다.

이번 버전은 DB에 nullable `animation_status` 열을 추가한다. 배포 전 기존처럼
DB를 백업한다. 기동 시 전체 이미지 검사를 하지 않고, 기존 미판별 GIF·WebP를
처음 조회할 때 결과를 저장한다. 새 이미지는 등록 시 판별한다. 정적 파일이나
읽을 수 없는 파일은 배지를 표시하지 않으며, 실패 결과도 저장해 반복 검사를
막는다. 판별은 원본·썸네일·파일 수정 시각을 변경하지 않는다.
사용자가 `bd6e13a4487b` 산출물의 테스트 완료와 적용을 확인했다.
해당 기능 커밋은 `main` 운영 기준에 포함하며, 아래 빌드 안내는 재빌드가
필요할 때 사용한다. 이 기록은 사용자 확인에 근거하며, 이번 문서 갱신에서
NAS 운영 컨테이너를 직접 검사하거나 재시작하지 않았다.

압축을 `/volume1/docker/test/gif-navigation/`에 풀었다면 다음과 같이 빌드한다.

```sh
cd /volume1/docker/test/gif-navigation/svrx-pinry
feature_revision="$(sed -n 's/^source_commit=//p' BUILD_INFO | cut -c1-12)"
bash build-image.sh "svrx-pinry:gif-navigation-${feature_revision}"
```

기존 볼륨·환경 설정·포트는 유지하고, 테스트할 프로젝트의 이미지 태그만
새 태그로 변경한다. 이전 이미지는 테스트가 끝날 때까지 보관한다.
실행 후 움직이는 GIF·WebP에만 배지가 나타나는지, 정적 GIF·WebP에는 없는지,
...
사용자가 `2b4bff7be6b8` 산출물로 만든 이미지의 테스트·운영 배포를 완료했고
현재까지 문제없다고 보고했다. 이 기록은 사용자 확인에 근거하며 이번 문서
갱신에서 운영 컨테이너를 직접 검사하거나 재시작하지 않았다.
실제 Synology NAS 재부팅 이후의 정상 기동은 아직 확인 전이다.
현대화 작업은 계속 보류하며, 아래 과거 검증 범위와 실환경 재부팅 확인을 구분한다.

당시 기동 복구 패키지는 운영 기준 `64388c7`에 기동 복구 수정만 적용한 별도 패치 작업선이다.
현대화용 의존성 업그레이드·DB 스키마 변경·미디어 재구성은 포함하지 않는다.
완성된 Docker 이미지 파일이 아니라 NAS에서 이미지를 만드는 빌드 소스다.

- 기동 작업자의 완료 보고 후 종료 대기를 1초에서 15초로 늘린다.
- 안전하게 복구할 수 있는 `worker_exit_timeout`, `gunicorn_start_failed` 상태에
  `서버 재시동` 버튼을 제공한다. 실제 자식 종료·시작 잠금·유지보수 게이트를
  확인한 경우에만 동작하며, NAS·Docker가 아닌 Pinry 기동 절차만 다시 시도한다.
- 공개 요청은 출처·일회성 자격 검증과 서버 전체 30초 간격·시작 관리자당
  최대 3회 제한을 적용한다. 설정 오류나 불확실한 데이터 상태는 강제 해제하지 않는다.
- 정상 서비스에서는 유지보수 페이지 요청을 메인 페이지로 바로 리디렉션한다.

기존 운영 데이터와 Compose 설정을 보존하고, 이미지만 교체한다.
산출물의 예제 Compose로 기존 운영 설정을 덮어쓰지 않는다.
기존에 멈춘 구 버전 컨테이너는 새 이미지 적용 전까지 이 버튼을 사용할 수 없다.

예를 들어 압축을 `/volume1/docker/test/startup-patch/`에 풀었다면 다음과 같이
기존 이미지 태그를 덮어쓰지 않는 별도 태그로 빌드한다.

```sh
cd /volume1/docker/test/startup-patch/svrx-pinry
patch_revision="$(sed -n 's/^source_commit=//p' BUILD_INFO | cut -c1-12)"
bash build-image.sh "svrx-pinry:startup-patch-${patch_revision}"
```

빌드 성공 후 Container Manager의 기존 프로젝트에서 `image:`만 위 태그로
변경하고 기존 볼륨·포트·환경 설정을 유지하여 적용한다. 실제 실행 버전은
관리자 화면의 빌드 정보에서 `BUILD_INFO`의 커밋 앞 12자리와 비교한다.

### 패치 검증 범위

기동·복구 Python 회귀 194개 중 191개 통과(로컬 Linux/root 전용 3개 제외),
화면 계약 25개와 패키지 회귀 54개를 통과했다. 추가로 NAS의 격리 컨테이너에서
Python 3.9.25·Nginx 1.22.1과 이번 패치 소스를 사용해 HTTP/TLS 재시도,
실제 종료 지연 후 복구, 제한·거부·리디렉션, 합성 데이터 보존을 확인했다.

이 검증은 운영 컨테이너 재시작이나 NAS의 주간 재부팅을 직접 수행한 결과가
아니다. 이후 사용자 이미지 테스트·운영 배포 확인은 위 운영 배포 기록에
추가했다. 다음 NAS 재부팅 결과는 여전히 별도로 확인해야 한다.

강제 종료로 내부 제어 소켓이 남은 경우 버튼을 안전하게 비활성화할 수 있다.
이 경우 웹 버튼으로 강제 해제하지 않으며, 관리자가 컨테이너 로그를 확인해야 한다.
...
레거시 `local_settings.py`에 인코딩 선언 없는 CP949 한글 주석만 있는 경우,
시작 시 원본을 `local_settings.py.encoding-*.bak`으로 별도 보존한 뒤 UTF-8로
자동 변환한다. 정상 UTF-8 및 올바른 인코딩 선언이 있는 파일은 변경하지 않는다.
설정 문자열·식별자에 비 ASCII 문자가 있거나 안전하게 판별할 수 없는 파일은
자동 변환하지 않는다. `local_settings_encoding_invalid`는 설정 파일의 인코딩,
`local_settings_syntax_invalid`는 파이썬 문법 확인이 필요하다는 뜻이다.
이 경우 원본을 백업하고 해당 문제를 수정한 뒤 시작 절차를 다시 시도한다.
오류 화면과 로그에는 설정 값이나 비밀 내용을 표시하지 않는다.
