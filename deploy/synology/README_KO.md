...
완료 ZIP의 다운로드 유효 기간은 완료 시각부터 **24시간**이다. 24시간은
실행 주기나 일일 횟수 제한이 아니므로 진행 중인 작업이 없으면 새 내보내기를
시작할 수 있다. 새 작업이 실패해도 이전 성공 ZIP은 원래 유효 기간까지
유지되고, 만료 ZIP과 오래된 작업 파일은 작업자가 보관 정책에 따라 정리한다.
...
스냅숏을 만들 수 없으면 부분 ZIP 없이 작업 전체를 실패로 표시한다. 화면의
**작업자 대기 중**은 전용 작업자 준비를 기다리는 상태다. 저장소 오류·용량
