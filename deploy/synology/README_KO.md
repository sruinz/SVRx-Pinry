...
상세보기 이전·다음 이동을 추가한다. 아래 기동 복구 이력의 수정도 포함하며,
현대화 작업·의존성 버전·DB 스키마·원본 파일은 변경하지 않는다.
아직 이 기능 추가 버전의 운영 배포가 확인된 것은 아니다.
...
실행 후 GIF·일반 이미지가 섞인 목록, 상세 좌우 이동, 목록 끝의 추가 로딩,
