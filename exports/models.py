...
from .contracts import (
    ...
    RECEIPT_LEVELS, SCOPE_VALUES, WORKER_HEALTH_STATES,
)
...
def _nonnegative(default=None, null=False):
    kwargs = {"validators": [MinValueValidator(0)], "null": null}
...
class ExportJob(models.Model):
    ...
    board_id_snapshot = models.PositiveIntegerField(null=True)
    board_name_snapshot = models.CharField(max_length=128, null=True)
    board_private_snapshot = models.NullBooleanField()
    board_owner_username_snapshot = models.CharField(max_length=150, null=True)
    ...
    progress_at = models.DateTimeField(null=True)
    ...
    snapshot_generation = models.UUIDField(null=True)
    snapshot_relative_path = models.CharField(max_length=512, null=True)
    snapshot_dir_dev = _nonnegative(null=True)
    snapshot_dir_ino = _nonnegative(null=True)
    snapshot_dir_uid = models.PositiveIntegerField(null=True)
    snapshot_dir_gid = models.PositiveIntegerField(null=True)
    snapshot_dir_mode = models.PositiveIntegerField(null=True)
    candidate_snapshot_generation = models.UUIDField(null=True)
    candidate_snapshot_relative_path = models.CharField(max_length=512, null=True)
    candidate_snapshot_dir_dev = _nonnegative(null=True)
    candidate_snapshot_dir_ino = _nonnegative(null=True)
    candidate_snapshot_dir_uid = models.PositiveIntegerField(null=True)
    candidate_snapshot_dir_gid = models.PositiveIntegerField(null=True)
    candidate_snapshot_dir_mode = models.PositiveIntegerField(null=True)
    ...
    verifying_sha256 = models.CharField(max_length=64, null=True)
    verifying_completed_at = models.DateTimeField(null=True)
    resume_count = models.PositiveSmallIntegerField(default=0)
    error_code = models.CharField(max_length=64, null=True, choices=_choices(ERROR_CONTRACTS))
    error_class = models.CharField(max_length=32, null=True, choices=_choices(ERROR_CLASSES))
    error_retryable = models.NullBooleanField()
    ready_relative_path = models.CharField(max_length=512, null=True)
    ready_display_name = models.CharField(max_length=255, null=True)
    ready_size = _nonnegative(null=True)
    ready_sha256 = models.CharField(max_length=64, null=True)
    ready_dev = _nonnegative(null=True)
    ready_ino = _nonnegative(null=True)
    ready_uid = models.PositiveIntegerField(null=True)
    ready_gid = models.PositiveIntegerField(null=True)
    ready_mode = models.PositiveIntegerField(null=True)
    ready_nlink = models.PositiveIntegerField(null=True)
    ...
    def clean(self):
        ...
        if self.state == "expired" and self.ready_cleanup_state not in ("pending", "blocked", "cleaned"):
...
class ExportBlob(models.Model):
    ...
    expected_sha256 = models.CharField(max_length=64, null=True)
    ...
    part_relative_path = models.CharField(max_length=512, null=True)
    snapshot_relative_path = models.CharField(max_length=512, null=True)
    capture_method = models.CharField(max_length=16, null=True)
    ...
    receipt_uid = models.PositiveIntegerField(null=True)
    receipt_gid = models.PositiveIntegerField(null=True)
    receipt_mode = models.PositiveIntegerField(null=True)
    receipt_nlink = models.PositiveIntegerField(null=True)
    receipt_mtime_ns = _nonnegative(null=True)
    receipt_ctime_ns = _nonnegative(null=True)
    receipt_sha256 = models.CharField(max_length=64, null=True)
...
class ExportItem(models.Model):
    ...
    referer_url = models.CharField(max_length=2048, null=True)
    referer_url_redacted = models.BooleanField(default=False)
    original_filename = models.CharField(max_length=255)
    archive_image_path = models.CharField(max_length=512, null=True)
    archive_xmp_path = models.CharField(max_length=512, null=True)
    inclusion_state = models.CharField(max_length=16, default="included", choices=_choices(INCLUSION_STATES))
    exclusion_reason = models.CharField(max_length=32, null=True)
...
class ExportSlot(models.Model):
    ...
    current_job = models.ForeignKey(ExportJob, null=True, on_delete=models.SET_NULL)
...
class ExportAttempt(models.Model):
    ...
    dir_uid = models.PositiveIntegerField(null=True)
    dir_gid = models.PositiveIntegerField(null=True)
    dir_mode = models.PositiveIntegerField(null=True)
...
class ExportAttemptFile(models.Model):
    ...
    relative_path = models.CharField(max_length=512)
    intent_relative_path = models.CharField(max_length=512, null=True)
    ...
    class Meta:
        ...
        constraints = (
            ...
            models.CheckConstraint(check=~Q(state__in=("ready_candidate", "published", "cleaned")) | Q(intent_relative_path__isnull=True), name="export_attempt_file_final_intent_valid"),
    ...
    def clean(self):
        ...
        if self.state in ("ready_candidate", "published", "cleaned") and self.intent_relative_path:
