        ...
        if isinstance(value, str):
            if value.lstrip().startswith('['):
                try:
                    ...
                    raise ValidationError(gettext('대괄호와 따옴표 없이 한 줄에 하나씩 입력하세요.')) from None
            else:
                value = value.splitlines()
        if not isinstance(value, list) or any(not isinstance(item, str) for item in value):
            raise ValidationError(gettext('한 줄에 하나의 문자열을 입력하세요.'))
...
class AuthPolicyAdminForm(forms.ModelForm):
    ...
    recovery_allowed_cidrs = LineListField(
        label=gettext_lazy('복구 허용 CIDR'),
        help_text=gettext_lazy('선택 · 한 줄에 하나씩 입력하세요. 예: 192.168.0.0/24 또는 10.0.0.0/8. 자신의 내부망만 허용하세요. 비우면 허용 대역이 없습니다.'))
    recovery_denied_cidrs = LineListField(
        label=gettext_lazy('복구 차단 CIDR'),
        help_text=gettext_lazy('선택 · 한 줄에 하나씩 입력하세요. 예: 192.168.0.45/32. 허용 대역에 포함되어도 차단이 우선합니다.'))

    class Meta:
        model = AuthPolicy
        fields = POLICY_FIELDS

        labels = {'allow_new_registrations': gettext_lazy('회원 가입 허용'),
                  'public_pins_enabled': gettext_lazy('비로그인 Pin 보기'),
                  'password_login_enabled': gettext_lazy('비밀번호 로그인 허용'),
                  'api_tokens_enabled': gettext_lazy('API 토큰 인증 허용'),
                  'recovery_allowed_cidrs': gettext_lazy('복구 허용 CIDR'),
                  'recovery_denied_cidrs': gettext_lazy('복구 차단 CIDR')}
        help_texts = {'allow_new_registrations': gettext_lazy('끄면 관리자나 기존 SSO 계정 연결로만 계정을 만들 수 있습니다.'),
                      'public_pins_enabled': gettext_lazy('끄면 로그인하지 않은 사용자는 Pin·보드·태그 API를 볼 수 없습니다. 로그인과 관리자 설정은 계속 사용할 수 있습니다.'),
                      'password_login_enabled': gettext_lazy('끄기 전에 관리자 SSO 로그인을 확인하세요. 내부망 IP 직접 접속에서는 관리자 복구 로그인을 사용할 수 있습니다.'),
                      'api_tokens_enabled': gettext_lazy('끄면 기존 토큰을 보존하면서 인증·노출·발급을 모두 차단합니다.')}
...
class SSOProviderAdminForm(forms.ModelForm):
    client_secret = forms.CharField(label='Client Secret', required=False, widget=forms.PasswordInput,
                                    help_text=gettext_lazy('입력·교체만 가능합니다. 비워두면 기존 비밀을 유지합니다.'))
    expected_revision = forms.IntegerField(widget=forms.HiddenInput)
    expected_policy_revision = forms.IntegerField(widget=forms.HiddenInput)
    allowed_endpoint_origins = LineListField(
        label=gettext_lazy('허용 서버 주소 (origin)'),
        help_text=gettext_lazy('한 줄에 하나씩 HTTPS 서버 주소만 입력하세요(경로 제외). 예: https://auth.example.com. 기본 제공자는 신규 등록 시 기본값을 사용합니다.'))
    internal_cidrs = LineListField(
        label=gettext_lazy('내부 IdP 허용 CIDR'),
        help_text=gettext_lazy('선택 · IdP가 내부망에 있을 때만 필요한 대역을 한 줄씩 입력하세요. 예: 192.168.0.45/32. 복구 로그인 허용 대역과는 별개입니다.'))
    ...
    class Meta:
        ...
        labels = dict(zip(PROVIDER_FIELDS, (
            gettext_lazy('제공자 종류'), gettext_lazy('표시 이름'), gettext_lazy('표시 순서'),
            gettext_lazy('활성화'), gettext_lazy('공개 서비스 기준 URL'), gettext_lazy('발급자'),
            'Discovery URL', gettext_lazy('Microsoft 테넌트 UUID'), 'Client ID',
            gettext_lazy('허용 endpoint origin'), gettext_lazy('내부 IdP 허용 CIDR'),
            gettext_lazy('신규 SSO 사용자 가입 허용'),
        )))
        help_texts = {'issuer': gettext_lazy('발급자·Client ID·비밀 변경 후 기존 연결과 관리자 로그인 확인을 재검증하세요.'),
                      'allowed_endpoint_origins': gettext_lazy('Google·Microsoft·GitHub 신규 등록은 빈 목록에 기본 origin을 적용합니다.'),
                      'allow_signup': gettext_lazy('기본은 기존 사용자 연결만 허용합니다. 신규 계정에는 관리자 권한을 부여하지 않습니다.')}
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['public_base_url'].help_text = gettext_lazy('활성화 시 필수 · 사용자가 로그인할 Pinry의 HTTPS 주소입니다. 내부망 접속 주소와 다를 수 있습니다. 경로는 넣지 마세요.')
        self.fields['public_base_url'].widget.attrs['placeholder'] = 'https://pins.example.com'
        self.fields['name'].help_text = gettext_lazy('필수 · 로그인 버튼에 표시할 이름입니다. 예: 회사 계정')
        self.fields['name'].widget.attrs['placeholder'] = gettext_lazy('회사 계정')
        self.fields['position'].help_text = gettext_lazy('필수 · 작은 숫자가 먼저 표시됩니다. 예: 0, 10, 20')
        self.fields['enabled'].help_text = gettext_lazy('처음에는 끈 상태로 저장해 확정 콜백 URI를 발급받고, 제공자 등록을 마친 후 활성화하세요.')
        self.fields['issuer'].help_text = gettext_lazy('Discovery에서 자동 확인합니다. 수동 설정 시 문서의 issuer 값과 마지막 슬래시까지 일치해야 합니다.')
        self.fields['discovery_url'].help_text = gettext_lazy('제공자의 OpenID 구성 URL을 붙여넣으세요. 발급자와 서버 주소는 자동 확인합니다.')
        self.fields['tenant_id'].help_text = gettext_lazy('활성화 시 필수 · Entra의 디렉터리(테넌트) ID입니다. UUID만 허용하며 common은 사용할 수 없습니다.')
        self.fields['tenant_id'].widget.attrs['placeholder'] = '11111111-2222-4333-8444-555555555555'
        self.fields['client_id'].help_text = gettext_lazy('활성화 시 필수 · 제공자의 앱 등록 화면에서 발급받은 클라이언트 ID입니다.')
        self.fields['client_secret'].widget.attrs['placeholder'] = gettext_lazy('제공자에서 발급한 비밀 값 (저장된 값은 표시하지 않음)')
        ...
        for field in self.fields.values():
            field.error_messages['required'] = gettext_lazy('이 항목을 입력하세요.')
            if isinstance(field, forms.URLField):
                field.error_messages['invalid'] = gettext_lazy('https://로 시작하는 올바른 URL을 입력하세요.')
    ...
    def clean(self):
        ...
        if kind == 'microsoft' and (enabled or data.get('tenant_id')):
            try:
                ...
                self.add_error('tenant_id', gettext('11111111-2222-4333-8444-555555555555 형식의 테넌트 UUID를 입력하세요.'))
        if enabled:
            if not data.get('client_id'):
                self.add_error('client_id', gettext('활성화하려면 Client ID가 필요합니다.'))
            if not data.get('client_secret') and not self.instance.encrypted_client_secret:
                self.add_error('client_secret', gettext('활성화하려면 Client Secret이 필요합니다.'))
            if not data.get('allowed_endpoint_origins') and not (self.instance._state.adding and PRESET_ORIGINS.get(kind)):
                self.add_error('allowed_endpoint_origins', gettext('활성화하려면 허용 서버 주소가 필요합니다.'))
        return data
    ...
    def _discover(self, data, kind):
        ...
        try:
            ...
            if _origin(parsed) not in origin_keys:
                raise ValidationError(gettext('Discovery 서버가 고급 설정의 허용 주소에 없습니다. 허용 주소를 비워 자동 설정하거나 수정하세요.'))
            ...
            if explicit_issuer and data['issuer'] != metadata['issuer']:
                raise ValidationError(gettext('입력한 발급자가 Discovery 문서와 다릅니다. 발급자를 비워 자동 확인하세요.'))
            if _origin(issuer) not in origin_keys:
                raise ValidationError(gettext('Discovery 발급자 서버가 허용 주소에 없습니다.'))
            for name in ('authorization_endpoint', 'token_endpoint', 'jwks_uri'):
                endpoint = _parse_url(metadata.get(name))
                if _origin(endpoint) not in origin_keys:
                    raise ValidationError(gettext('다른 서버의 인증 주소는 고급 설정에서 발급자와 허용 서버 주소를 확인하세요.'))
        ...
        for name in ('public_base_url', 'issuer', 'discovery_url'):
            ...
            if not value:
                if required and name not in self.errors:
                    self.add_error(name, gettext('활성화하려면 HTTPS 기준 URL을 입력하세요.'))
                continue
            try:
                ...
                valid = False
            if not valid:
                self.add_error(name, gettext('사용자 정보·쿼리 없는 HTTPS URL을 입력하세요. 공개 서비스 주소에는 경로를 넣지 마세요.'))
    ...
    def clean_allowed_endpoint_origins(self):
        ...
        for value in origins:
            try:
                ...
                valid = False
            if not valid:
                raise ValidationError(gettext('경로 없이 HTTPS 서버 주소를 한 줄에 하나씩 입력하세요.'))
        return origins
...
class AuthPolicyAdmin(ActiveSuperuserAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        (gettext_lazy('사이트 접근'), {'fields': ('allow_new_registrations', 'public_pins_enabled')}),
        (None, {'fields': ('password_login_enabled', 'api_tokens_enabled', 'expected_revision')}),
        (gettext_lazy('기존 전용 복구 포트 고급 설정 (내부망 직접 복구에는 불필요)'), {
...
class SSOProviderAdmin(ActiveSuperuserAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (gettext_lazy('고급 설정 (일반적으로 변경할 필요 없음)'), {'classes': ('collapse',), 'fields': (
    ...
    def setup_guide(self, request):
        ...
        return TemplateResponse(request, 'admin/users/ssoprovider/setup_guide.html', {
            **self.admin_site.each_context(request), 'title': gettext('SSO 설정 가이드'),
    ...
    @admin.display(description=gettext_lazy('등록할 정확한 콜백 주소'))
    def callback_address(self, obj):
        if obj is None or obj._state.adding:
            return gettext('비활성 상태로 저장 후 확정 콜백 주소를 확인하세요.')
        try:
            ...
            return gettext('공개 서비스 HTTPS URL을 저장하면 복사할 콜백 주소가 표시됩니다.')
