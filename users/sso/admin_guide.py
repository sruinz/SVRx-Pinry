...
PROVIDER_GUIDES = {
    'authentik': {
        ...
        'client_example': _('Authentik에서 발급한 Client ID'),
        'steps': [_('Applications에서 OAuth2/OpenID 제공자와 애플리케이션을 만드세요.'),
                  _('Confidential 클라이언트를 사용하고 아래 확정 리디렉션 URI를 정확히 등록하세요.'),
                  _('Client ID·Client Secret·Discovery URL을 입력하세요. 발급자는 자동 확인합니다.')],
        'note': _('전역 issuer 모드에서는 발급자와 Discovery 경로가 다릅니다. 애플리케이션의 Discovery URL을 직접 입력하세요.'),
    ...
    'synology': {
        ...
        'client_example': _('SSO Server의 응용 프로그램 ID'),
        'steps': [_('SSO Server에서 OIDC 서버를 활성화하세요.'),
                  _('OIDC 응용 프로그램을 추가하고 아래 확정 리디렉션 URI를 등록하세요.'),
                  _('응용 프로그램 ID·비밀·Well-known URL을 입력하세요. 발급자는 자동 확인합니다.')],
        'note': _('주소 예시는 설치 환경에 따라 다릅니다. SSO Server가 표시하는 실제 Well-known URL을 우선 사용하세요.'),
        'docs': 'https://kb.synology.com/en-global/DSM/help/SSOServer/sso_server_application_list?version=7',
    },
    'oidc': {
        'label': _('범용 OIDC'), 'issuer_example': 'https://idp.example.com/realms/pinry',
        'discovery_example': 'https://idp.example.com/realms/pinry/.well-known/openid-configuration',
        'client_example': 'pinry',
        'steps': [_('제공자에서 서버용 OIDC 클라이언트를 만드세요.'),
                  _('아래 확정 리디렉션 URI와 openid profile email 범위를 등록하세요.'),
                  _('Client ID·비밀·Discovery URL을 입력하세요. 발급자는 자동 확인합니다.')],
        'note': _('현재 Pinry는 PKCE S256을 지원하는 제공자가 필요합니다. issuer는 문서와 마지막 슬래시까지 같아야 합니다.'),
    ...
    'google': {
        ...
        'steps': [_('Google Cloud에서 OAuth 동의 화면과 웹 애플리케이션 클라이언트를 설정하세요.'),
                  _('승인된 리디렉션 URI에 아래 확정 주소를 등록하세요.'),
                  _('Client ID와 Client Secret을 입력하세요. 테스트 모드라면 테스트 사용자도 등록하세요.')],
        'note': _('발급자와 Discovery URL은 자동 적용됩니다.'),
    ...
    'microsoft': {
        ...
        'steps': [_('Entra 관리 센터에서 앱을 등록하고 웹 플랫폼을 선택하세요.'),
                  _('아래 확정 리디렉션 URI와 애플리케이션(클라이언트) ID, 디렉터리(테넌트) ID를 확인하세요.'),
                  _('클라이언트 비밀을 만들고 비밀 ID가 아닌 비밀 값을 입력하세요.')],
        'note': _('현재 Pinry는 명시적인 테넌트 UUID를 사용합니다. common·organizations·도메인 이름은 입력하지 마세요.'),
    ...
    'github': {
        ...
        'steps': [_('GitHub Settings → Developer settings → OAuth Apps에서 앱을 만드세요.'),
                  _('Homepage URL에는 Pinry 공개 주소, Authorization callback URL에는 아래 확정 주소를 등록하세요.'),
                  _('Client ID와 생성한 Client Secret을 입력하세요.')],
        'note': _('GitHub OAuth 로그인에는 Discovery URL이 없습니다. read:user user:email 범위를 사용합니다.'),
