...
설정 화면은 제공자 종류에 필요한 입력란과 예시를 표시한다. CIDR과 허용
서버 주소는 대괄호·따옴표 없이 한 줄에 하나씩 입력한다. 관리자 목록·설정의
...
기존 사용자는 로그인한 뒤 프로필에서 SSO 계정을 명시적으로 연결한다.
이메일이나 사용자 이름이 같다는 이유로 자동 연결하지 않는다. 연결·해제 전
최근 재인증이 필요하며, SSO 재인증은 새 state·nonce·PKCE 왕복으로
연결된 외부 ID를 다시 확인하는 의미다. IdP에 유효한 세션이 있으면
비밀번호나 MFA 입력 없이 완료될 수 있으며, 모든 IdP의 MFA 재입력을
강제하는 기능은 아니다.

비밀번호 로그인을 끄기 전에 현재 제공자의 슈퍼 관리자 SSO 연결·로그인과
현재 복구 배포의 최근 복구 로그인을 모두 확인해야 한다. API 토큰
정책은 별도로 제어된다. 비활성화하면 기존 토큰을 삭제하지 않고
노출·발급·인증을 차단하며, 다시 허용하면 기존 자동화가 회복된다.

복구 포트는 기본으로 공개되지 않는다. 설치자가 전용 TLS 인증서·키,
직접 원본 IP가 보존되는 LAN 경로, 허용/차단 CIDR을 모두 검증한 뒤에만
활성화한다. NAS·게이트웨이·프록시 주소는 차단하고, NAT로 외부
요청과 내부 클라이언트를 구별할 수 없으면 복구를 열지 않는다. 자세한
